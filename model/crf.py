...
class CRF(nn.Module):
    ...
    def _viterbi_decode_nbest(self, feats, mask, nbest):
        ...
        partition_history.append(partition.view(
            batch_size, tag_size, 1).expand(batch_size, tag_size, nbest))
        # iter over last scores
        for idx, cur_values in seq_iter:
            if idx == 1:
                cur_values = cur_values.view(batch_size, tag_size, tag_size) + partition.contiguous(
                ).view(batch_size, tag_size, 1).expand(batch_size, tag_size, tag_size)
            else:
                # previous to_target is current from_target
        ...
        pad_zero = autograd.Variable(torch.zeros(
            batch_size, tag_size, nbest)).long()
        ...
        minus_scores = scores - \
            max_scores.view(batch_size, 1).expand(batch_size, nbest)
